...
switch ($accion) {
    ...
    case 'registro':
        if (isset($_POST['enviar'])) {
            ...
            if ($resultado === 1) {
                $nombreCompleto = trim(($data['nombre'] ?? '') . ' ' . ($data['primer_apellido'] ?? ''));
                $asunto = 'Bienvenido a Literan';
                $cuerpo = '<h2>Tu cuenta fue creada exitosamente</h2>'
                    . '<p>Hola ' . htmlspecialchars($nombreCompleto, ENT_QUOTES, 'UTF-8') . ', gracias por registrarte en Literan.</p>'
                    . '<p>Tu contraseña es: <strong>' . htmlspecialchars($data['contrasena'], ENT_QUOTES, 'UTF-8') . '</strong></p>'
                    . '<p>Ya puedes iniciar sesion con tu correo.</p>';

                $app->envioCorreo($nombreCompleto, $data['correo'], $asunto, $cuerpo);
